...
from trt_latentsync import NVTXContext, PROFILER_AVAILABLE
from trt_latentsync.backends.snn_predictor import SNNPredictor
from trt_latentsync.src.utils.status_code import PI_STATUS_CODE
...
def video_worker(audio_files, args):
    ...
    # 初始化SNNPredictor - 使用model_path目录下的配置文件
    config_file = os.path.join(args.model_path, "config.yaml")
    if not os.path.exists(config_file):
        print(f"❌ Error: Config file not found: {config_file}")
        print(f"Please ensure config.yaml exists in the model_path directory: {args.model_path}")
        exit(1)
    ...
    if 'snn_torch_predictor' in config_data['predictors']:
        ...
        if 'config_file' in config_data['predictors']['snn_torch_predictor']:
            ...
            if not os.path.isabs(config_file_path):
                # 如果是相对路径，转换为绝对路径
                config_file_path = os.path.join(args.model_path, config_file_path)
                config_data['predictors']['snn_torch_predictor']['config_file'] = config_file_path
        
        # 直接使用原始配置文件，命令行参数在SNNTorchBackend中处理
        # 使用绝对路径避免重复拼接
        config_data['predictors']['snn_torch_predictor']['config_file'] = os.path.join(args.model_path, "V1.0_latentsync_video_predictor.json")
        ...
        print(f"[INFO] Using business config: {os.path.join(args.model_path, 'V1.0_latentsync_video_predictor.json')}")
    ...
    # 临时保存更新后的配置到model_path目录
    temp_config_file = os.path.join(args.model_path, "temp_config.yaml")
