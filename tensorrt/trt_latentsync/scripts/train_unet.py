...
# from trt_latentsync.data.unet_dataset import UNetDataset
from trt_latentsync.data.hy_unet_384_dataset import CustomUNetDataset_384_hy_0227_v1 as UNetDataset
# from trt_latentsync.data.hy_unet_384_dataset import CustomUNetDataset_384_hy_0227_v2 as UNetDataset
from trt_latentsync.models.unet import UNet3DConditionModel
from trt_latentsync.models.syncnet import SyncNet
from trt_latentsync.pipelines.lipsync_pipeline import LipsyncPipeline
from trt_latentsync.utils.util import (
    init_dist,
    cosine_loss,
    reversed_forward,
)
from trt_latentsync.utils.util import plot_loss_chart, gather_loss
from trt_latentsync.whisper.audio2feature import Audio2Feature
from trt_latentsync.trepa import TREPALoss
...
def main(config):
    ...
    #     # 兼容新的多数据目录配置
    #     config.data.train_data_dirs[0] if hasattr(config.data, 'train_data_dirs') and len(config.data.train_data_dirs) > 0 else config.data.train_data_dir, 
    #     config
    # )
    ...
    # logger.info(f"***** Running first_epoch *****")
