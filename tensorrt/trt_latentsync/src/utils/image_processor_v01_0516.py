...
from trt_latentsync import NVTXContext
...
def load_fixed_mask(resolution: int, mask_path="latentsync/utils/mask.png") -> torch.Tensor:
...
class ImageProcessor:
    def __init__(self, resolution: int = 512, mask: str = "fix_mask", device: str = "cpu", mask_image=None, mask_path="latentsync/utils/mask.png"):
...
if __name__ == "__main__":
    ...
    # python -m latentsync.utils.image_processor
    image_processor = ImageProcessor(256, mask="fix_mask", device="cuda")
    video = cv2.VideoCapture("/home/work/houyi/pj_25_q1/LatentSync/assets/demo1_video.mp4")
    while True:
        ...
        # 直接使用numpy数组，而不是转换为张量
        print(f"frame shape: {frame.shape}")
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # 确保是RGB格式
        face, _, _ = image_processor.affine_transform(frame_rgb)
    ...
    cv2.imwrite("/home/work/houyi/pj_25_q1/LatentSync/latentsync/utils/face_ori_00.jpg", face)
