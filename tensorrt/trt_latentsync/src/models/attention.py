    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if self._use_memory_efficient_attention_xformers:
            ...
            if self._slice_size is None or query.shape[0] // self._slice_size == 1:
                # 使用 flash_attention.py 中的 flash_attention 函数替换 _attention
                # 数据格式保持 [B*N, L, C] 不变，直接传递
                hidden_states = flash_attention(query, key, value, num_heads=self.heads, attention_mask=attention_mask)
                # Flash Attention 返回 [B*N, L, D] 格式，需要转换为 [B, L, C] 格式给 to_out 层
                hidden_states = self.reshape_batch_dim_to_heads(hidden_states)
                # hidden_states = self._attention(query, key, value, attention_mask)
            else:
                hidden_states = self._sliced_attention(query, key, value, sequence_length, dim, attention_mask)
                # print("==> _sliced_attention")
