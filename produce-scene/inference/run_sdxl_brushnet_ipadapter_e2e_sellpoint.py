...
def rewrite_position_params(template_config, new_position):
    # 根据需求修改前景大小与位置
    # xxx
    template_config = json.loads(template_config)
    template_config['extra_info']['target_center'] = [0.5, 0.5]
    template_config['extra_info']['target_h'] = 0.9
    template_config['extra_info']['target_w'] = 0.9
...
def process_row(row):
    ...
    fg_url = global_url_prefixed + '/' + fg_rgba_id
    ...
    # 使用自定义前景大小与位置
    template_config = rewrite_position_params(template_config, [])
...
if __name__ == '__main__':
    ...
    # init model
    runner = SDXLBrushNetIpadapterRunner(e2e_config['model']['sdxl_ckpt'], 
                                         e2e_config['model']['brushnet_ckpt'],
                                         e2e_config['model']['ipadapter_ckpt'])
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        preprocess_results = [[row[product_fg_key], executor.submit(process_row, row)] for _, row in df.iterrows()]
    ...
    pd.DataFrame(outputs, columns=columns).to_csv(
        os.path.join(args.logdir, args.csv.split('/')[-1].replace('.csv', csv_suffix)), index=False)
