...
resume_module_root=./ckpts/t2i/model/pytorch_model_module.pt # checkpoint root for model resume
resume_ema_root=./ckpts/t2i/model/pytorch_model_ema.pt      # checkpoint root for ema resume
index_file=dataset/porcelain/jsons/porcelain3.json             # index file for dataloader
...
ckpt_every=5000                                            # create a ckpt every a few steps.
ckpt_latest_every=400000                                     # create a ckpt named `latest.pt` every a few steps.
ckpt_every_n_epoch=1                                          # create a ckpt every a few epochs.
epochs=1                                                      # total training epochs


sh $(dirname "$0")/run_g_lcm.sh \
    --task-flag ${task_flag} \
