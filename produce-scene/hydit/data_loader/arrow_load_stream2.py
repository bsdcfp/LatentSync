...
class TextImageArrowStream(Dataset):
    ...
    def split_rgba_to_rgb_images(self, rgba_image):
        """
        将RGBA格式的图片拆分成原始RGB图片和基于mask处理后的RGB图片，并分别保存

        :param rgba_image_path: RGBA图片的路径
        :param original_rgb_output_path: 保存原始RGB图片的路径
        :param masked_rgb_output_path: 保存基于mask处理后的RGB图片的路径
        """
        ...
        # 提取原始RGB通道数据（前三个通道）
        original_rgb_array = rgba_array[:, :, :3]

        # 提取Alpha通道（透明度通道）作为mask数据
        alpha_channel = rgba_array[:, :, 3]

        # 创建一个全0的数组，用于构建mask后的RGB数组，形状与原始RGB数组相同
        masked_rgb_array = original_rgb_array.copy() 

        # 根据Alpha通道的值，将原始RGB数据复制到masked_rgb_array中
        for row in range(height):
            for col in range(width):
                if alpha_channel[row, col] != 255:
                    masked_rgb_array[row, col, :] = 0

        # 将原始RGB数组转换为PIL图像对象并保存
        original_rgb_image = Image.fromarray(original_rgb_array)

        # 将mask后的RGB数组转换为PIL图像对象并保存
        masked_rgb_image = Image.fromarray(masked_rgb_array)
        mask = Image.fromarray(alpha_channel)
        return original_rgb_image, masked_rgb_image, mask
    ...
    def get_raw_image(self, index, image_key="image"):
        try:
            ...
            ret = Image.new("RGBA", (256, 256), (255, 255, 255, 0))
        return ret

    @staticmethod
    def random_crop_image(image, mask_rgb, mask, origin_size, target_size):
        ...
        mask_rgb = mask_rgb.resize((new_width, new_height), Image.LANCZOS)
        ...
        mask_rgb_crop = mask_rgb.crop((x_start, y_start, x_start + target_size[0], y_start + target_size[1]))
        mask_crop = mask.crop((x_start, y_start, x_start + target_size[0], y_start + target_size[1]))
        crops_coords_top_left = (x_start, y_start)
        return image_crop, mask_rgb_crop, mask_crop, crops_coords_top_left
    ...
    def get_image_with_hwxy(self, index, image_key="image"):
        ...
        rgb_image, masked_rgb_image, mask = self.split_rgba_to_rgb_images(image)
        ...
        if self.multireso:
            ...
            image_crop, mask_rgb_crop, mask_crop, crops_coords_top_left = self.random_crop_image(rgb_image, masked_rgb_image, mask, origin_size, target_size)
            image_tensor = self.flip_norm(image_crop)
            mask_rgb_tensor = self.flip_norm(mask_rgb_crop)
            mask_tensor = self.flip_norm_mask(mask_crop)
        ...
        return image_tensor, mask_rgb_tensor, mask_tensor, kwargs
    ...
    def __getitem__(self, ind):
        ...
        original_pil_image, mask_rgb_tensor, mask_tensor, kwargs = self.get_image_with_hwxy(ind)
