...
from hydit.data_loader.arrow_load_stream2 import TextImageArrowStream
...
from hydit.modules.models_reference import HUNYUAN_DIT_MODELS, HunYuanReferenceNet
...
def prepare_model_inputs(args, batch, device, vae, text_encoder, text_encoder_t5, freqs_cis_img):
    image, mask_rgb, mask, text_embedding, text_embedding_mask, text_embedding_t5, text_embedding_mask_t5, kwargs = batch
    ...
    condition_latents = vae.encode(mask_rgb).latent_dist.sample().mul_(vae_scaling_factor)
    ...
    condition_latents = torch.concat([condition_latents, mask], dim=1)
...
def main(args):
    ...
    with deepspeed.zero.Init(data_parallel_group=torch.distributed.group.WORLD,
        ...
        controlnet = HunYuanReferenceNet(args,
                                       input_size=latent_size,
