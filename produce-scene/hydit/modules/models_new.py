...
def fed_forward(referencemodel, hunyuandit,
                x,
                t,
    ...
    x_ref, c_ref, freqs_cis_img_ref = referencemodel.half_forward(x, t, image_meta_size, cos_cis_img, sin_cis_img)
    x, c, text_states, freqs_cis_img = hunyuandit.half_forward(x, t, image_meta_size, cos_cis_img, sin_cis_img)
    skips_ref = []
    skips = []
    for i in range(len(referencemodel.blocks)):
        if i != 0:
            ref_in = merge2(x_ref, x)
        else:
            ref_in = x_ref
        if i > len(referencemodel.blocks) // 2:
            skip_ref = skips_ref.pop()
            x_ref = referencemodel.blocks[i](ref_in, c_ref, None, freqs_cis_img_ref, skip_ref) 
            hunyuan_in = merge1(x, x_ref)
            x = hunyuandit.blocks[i](hunyuan_in, c, None, freqs_cis_img, skip)
        else:
            x_ref = referencemodel.blocks[i](ref_in, c_ref, None, freqs_cis_img_ref)
            hunyuan_in = merge1(x, x_ref)
            x = hunyuandit.blocks[i](hunyuan_in, c, None, freqs_cis_img)
        if layer < (self.depth // 2 - 1):
            skips_ref.append(x_ref)
            skips.append(x)
...
class HunYuanDiT(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def half_forward(self,
        ...
        return x, c, text_states, freqs_cis_img
    ...
    def forward(self,
        ...
        x, c, text_states, freqs_cis_img = self.half_forward(x, 
        t, encoder_hidden_states=None, 
        text_embedding_mask=None, 
        encoder_hidden_states_t5=None,
        text_embedding_mask_t5=None,
        image_meta_size=None,
...
class ReferenceNet(ModelMixin, ConfigMixin, PeftAdapterMixin):
    @register_to_config
    def __init__(self,
                 args: Any,
                 input_size: tuple = (32, 32),
...
class HunYuanDiT_New(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    HunYuanDiT: Diffusion model with a Transformer backbone.
    ...
    """
    @register_to_config
    def __init__(self, args, hunyuanditmodel, referencemodel, hidden_size):
        super().__init__()
        self.args = args
        self.hidden_size = hidden_size
        self.hunyuanditmodel = hunyuanditmodel
    ...
    def forward(self,
        ...
        x, c, text_states, freqs_cis_img = self.hunyuanditmodel.half_forward(x, 
        t, encoder_hidden_states=None, 
        text_embedding_mask=None, 
        encoder_hidden_states_t5=None,
        text_embedding_mask_t5=None,
