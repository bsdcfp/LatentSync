...
from aigc_lightning_kernel.transformers.apply_2d_rope import AIGCLightning2DRope
...
try:
    ...
    logger.error(f'flash_attn import failed: {e}')
...
class FlashSelfMHAModified(nn.Module):
    ...
    def forward(self, x, freqs_cis_img=None):
        ...
        """
        b, s, d = x.shape
        logger.debug(f"[FlashSelfMHAModified] Input tensor shape: batch={b}, seq_len={s}, hidden_dim={d}")
        with NVTXContext("FlashSelfMHAModified.forward.proj"):
            qkv = self.Wqkv(x)
            qkv = qkv.view(b, s, 3, self.num_heads, self.head_dim)  # [b, s, 3, h, d]
        with NVTXContext("FlashSelfMHAModified.forward.unbind"):
            q, k, v = qkv.unbind(dim=2) # [b, s, h, d]
        
        with NVTXContext("FlashSelfMHAModified.forward.qk_norm"):
            q = self.q_norm(q).half()   # [b, s, h, d]
            k = self.k_norm(k).half()

        # Apply RoPE if needed
        if freqs_cis_img is not None:
            # logger.info(f"[FlashSelfMHAModified] Applying RoPE cos shape: {freqs_cis_img[0].shape}, sin shape: {freqs_cis_img[1].shape}")
            # qq, kk = apply_rotary_emb(q, k, freqs_cis_img)
            qq, kk = AIGCLightning2DRope(q, k, freqs_cis_img[0], freqs_cis_img[1])
            assert qq.shape == q.shape and kk.shape == k.shape, f'qq: {qq.shape}, q: {q.shape}, kk: {kk.shape}, k: {k.shape}'
            q, k = qq, kk
            
        # with NVTXContext("FlashSelfMHAModified.forward.stack"):
        #     qkv = torch.stack([q, k, v], dim=2)     # [b, s, 3, h, d]
        with NVTXContext("FlashSelfMHAModified.forward.inner_attn"):
            # context = self.inner_attn(qkv)
            context = flash_attn_func(q, k, v)
        with NVTXContext("FlashSelfMHAModified.forward.out_proj"):
            out = self.out_proj(context.view(b, s, d))
        with NVTXContext("FlashSelfMHAModified.forward.proj_drop"):
...
class FlashCrossMHAModified(nn.Module):
    ...
    def forward(self, x, y, freqs_cis_img=None):
        ...
        logger.debug(f"[FlashCrossMHAModified] Input tensor shapes: x=({b}, {s1}, {_}), y=({b}, {s2}, {_})")
        with NVTXContext("FlashCrossMHAModified.forward.q_proj"):
        ...
        with NVTXContext("FlashCrossMHAModified.forward.qk_norm"):
        ...
        if freqs_cis_img is not None:
            ...
            qq, _ = AIGCLightning2DRope(q, None, freqs_cis_img[0], freqs_cis_img[1])
            assert qq.shape == q.shape, f'qq: {qq.shape}, q: {q.shape}'
            q = qq                              # [b, s1, h, d]
        
        # with NVTXContext("FlashCrossMHAModified.forward.stack"):
        #     kv = torch.stack([k, v], dim=2)         # [b, s1, 2, h, d]
        with NVTXContext("FlashCrossMHAModified.forward.inner_attn"):
            # context = self.inner_attn(q, kv)        # [b, s1, h, d]
            context = flash_attn_func(q, k, v)
            context = context.view(b, s1, -1)       # [b, s1, D]

        with NVTXContext("FlashCrossMHAModified.forward.out_proj"):
            out = self.out_proj(context)
        with NVTXContext("FlashCrossMHAModified.forward.proj_drop"):
            out = self.proj_drop(out)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y, freqs_cis_img=None):
        ...
        logger.debug(f"[CrossAttention] Input tensor shapes: x=({b}, {s1}, {c}), y=({b}, {s2}, {c})")

        with NVTXContext("CrossAttention.forward.qkv_proj"):
            q = self.q_proj(x).view(b, s1, self.num_heads, self.head_dim)   # [b, s1, h, d]
            kv = self.kv_proj(y).view(b, s2, 2, self.num_heads, self.head_dim)    # [b, s2, 2, h, d]
            k, v = kv.unbind(dim=2) # [b, s, h, d]
        
        with NVTXContext("CrossAttention.forward.qk_norm"):
            q = self.q_norm(q)
            k = self.k_norm(k)
        ...
        with NVTXContext("CrossAttention.forward.matmul"):
            q = q * self.scale
        ...
        with NVTXContext("CrossAttention.forward.softmax"):
            attn = attn.softmax(dim=-1)                 # attn -> B, H, L1, L2
        with NVTXContext("CrossAttention.forward.attn_drop"):
            attn = self.attn_drop(attn)
        with NVTXContext("CrossAttention.forward.attn_mul_v"):
        ...
        with NVTXContext("CrossAttention.forward.out_proj"):
            out = self.out_proj(context)  # context.reshape - B, L1, -1
        with NVTXContext("CrossAttention.forward.proj_drop"):
            out = self.proj_drop(out)
...
class Attention(nn.Module):
    ...
    def forward(self, x, freqs_cis_img=None):
        B, N, C = x.shape
        logger.debug(f"[Attention] Input tensor shape: batch={B}, seq_len={N}, hidden_dim={C}")
        with NVTXContext("Attention.forward.Wqkv"):
            qkv = self.Wqkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)   # [3, b, h, s, d]
            q, k, v = qkv.unbind(0)     # [b, h, s, d]
        with NVTXContext("Attention.forward.layernorm"):
        ...
        with NVTXContext("Attention.forward.scale"):
            q = q * self.scale
        with NVTXContext("Attention.forward.attn"):
            attn = q @ k.transpose(-2, -1)              # [b, h, s, d] @ [b, h, d, s]
            attn = attn.softmax(dim=-1)                 # [b, h, s, s]
        with NVTXContext("Attention.forward.attn_drop"):
            attn = self.attn_drop(attn)
        with NVTXContext("Attention.forward.attn_mul_v"):
            x = attn @ v                                # [b, h, s, d]
            x = x.transpose(1, 2).reshape(B, N, C)      # [b, s, h, d]
        with NVTXContext("Attention.forward.out_proj"):
            x = self.out_proj(x)
        with NVTXContext("Attention.forward.proj_drop"):
            x = self.proj_drop(x)
