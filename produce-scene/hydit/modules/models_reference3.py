...
from aigc_lightning_kernel.transformers.layer_norm import AIGCLightningLayerNorm
...
class HunYuanDiTBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if norm_type == "layer":
            ...
            norm_layer = AIGCLightningLayerNorm
        elif norm_type == "rms":
            norm_layer = RMSNorm
        else:
    ...
    def _forward(self, x, c=None, text_states=None, freq_cis_img=None, skip=None):
        ...
        if self.skip_linear is not None:
            ...
            x = self.skip_linear(cat) # 线性变换降维

        # Self-Attention
        shift_msa = self.default_modulation(c).unsqueeze(dim=1)
        
        with NVTXContext("HunyuanDiTBlock.forward.self-attention"):
            attn_inputs = (
        ...
        # Cross-Attention
        with NVTXContext("HunyuanDiTBlock.forward.cross-attention"):
            cross_inputs = (
        ...
        # FFN Layer
        with NVTXContext("HunyuanDiTBlock.forward.ffn"):
...
class FinalLayer(nn.Module):
    ...
    def __init__(self, final_hidden_size, c_emb_size, patch_size, out_channels):
        ...
        self.norm_final = AIGCLightningLayerNorm(final_hidden_size, elementwise_affine=False, eps=1e-6)
...
class HunYuanReferenceNet(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def __init__(self,
        ...
        self.x_embedder = PatchEmbed(input_size, patch_size, conditioning_channels, hidden_size)
        ...
        self.blocks = nn.ModuleList([
            ReferenceBlock(hidden_size=hidden_size,
        ...
        self.fix_weight_modules = ['style_embedder', 'x_embedder', 't_embedder',]
    ...
    def from_dit(self, dit):
        ...
        x_embedder = torch.zeros((1408,5,2,2)) ##self.x_embedder.proj.weight.shape
        x_embedder[:,:4,...] = dit.x_embedder.proj.weight
        self.x_embedder.proj.weight = torch.nn.Parameter(x_embedder)
        self.x_embedder.proj.bias = dit.x_embedder.proj.bias
    ...
    def set_trainable(self):
        ...
        self.x_embedder.requires_grad_(True)
    ...
    def forward(self,
        ...
        # """
        with NVTXContext("HunyuanReferenceNet.forward.preprocess"):
        ...
        with NVTXContext("HunyuanReferenceNet.forward.t_embedder"):
            t = self.t_embedder(t)
        # x = self.x_embedder(x)

        # Get image RoPE embedding according to `reso`lution.
        with NVTXContext("HunyuanReferenceNet.forward.freqs_cis_img"):
            freqs_cis_img = (cos_cis_img, sin_cis_img)

        # ========================= Concatenate all extra vectors =========================
        # Build text tokens with pooling
        with NVTXContext("HunyuanReferenceNet.forward.pooler"):
        ...
        if image_meta_size is not None:
            with NVTXContext("HunyuanReferenceNet.forward.image_embedding"):
        ...
        if style is not None:
            with NVTXContext("HunyuanReferenceNet.forward.style_embedding"):
                style_embedding = self.style_embedder(style)
                extra_vec = torch.cat([extra_vec, style_embedding], dim=1)

        # Concatenate all extra vectors
        with NVTXContext("HunyuanReferenceNet.forward.cat_all_extra_embedder"):
            c = t + self.extra_embedder(extra_vec)  # [B, D]

        # ========================= Deal with Condition =========================
        # condition = torch.concat([x, condition], 1)
        with NVTXContext("HunyuanReferenceNet.forward.x_embedder"):
            condition = condition.half()
            x = self.x_embedder(condition)

        # ========================= Forward pass through HunYuanDiT blocks =========================
        controls = []
        skips = []
        # x = x #+ self.before_proj(condition) # add condition
        # text_states = torch.zeros((1,333,1024)).to(x.dtype).to(x.device)
        with NVTXContext("HunyuanReferenceNet.forward.blocks"):
            for layer, block in enumerate(self.blocks):
                if layer > self.depth // 2:
                    skip = skips.pop()
                    if x.shape[0] != skip.shape[0]:
                        print(f"[WARNING] Layer {layer}: batch_size mismatch - x.shape[0]={x.shape[0]}, skip.shape[0]={skip.shape[0]}")
                    x = block(x, c, text_states, freqs_cis_img, skip) 
                else:
                    x = block(x, c, text_states, freqs_cis_img)
                # if layer == 0 or layer == 1 or layer == 2:
...
class HunYuanDiT(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(self,
        ...
        #     [zero_module(nn.Linear(hidden_size * 2, hidden_size)) for _ in range(depth)]
        # )

        # self.norm_list = nn.ModuleList([FP32_Layernorm(2 * hidden_size, elementwise_affine=True, eps=1e-6) for _ in range(depth)])
    ...
    def forward(self,
        ...
        return_dict: bool
            Whether to return a dictionary.
        """
        with NVTXContext("HunyuanDiT.forward.preprocess"):
            text_states = encoder_hidden_states                     # Already CFG processed in pipeline
            text_states_t5 = encoder_hidden_states_t5               # Already CFG processed in pipeline
            text_states_mask = text_embedding_mask.bool()           # Already CFG processed in pipeline
            text_states_t5_mask = text_embedding_mask_t5.bool()     # Already CFG processed in pipeline
        ...
        # ========================= Build time and image embedding =========================
        with NVTXContext("HunyuanDiT.forward.t_embedder"):
            t = self.t_embedder(t)
        with NVTXContext("HunyuanDiT.forward.x_embedder"):
            x = self.x_embedder(x)
        # Get image RoPE embedding according to `reso`lution.
        with NVTXContext("HunyuanDiT.forward.freqs_cis_img"):
            freqs_cis_img = (cos_cis_img, sin_cis_img)

        # ========================= Concatenate all extra vectors =========================
        # Build text tokens with pooling
        with NVTXContext("HunyuanDiT.forward.pooler"):
            extra_vec = self.pooler(encoder_hidden_states_t5)
        ...
        # Build image meta size tokens if applicable
        if image_meta_size is not None:
            with NVTXContext("HunyuanDiT.forward.image_embedding"):
                image_meta_size = timestep_embedding(image_meta_size.view(-1), 256)   # [B * 6, 256]
                if self.args.use_fp16:
                    image_meta_size = image_meta_size.half()
                image_meta_size = image_meta_size.view(-1, 6 * 256)
        ...
        # Build style tokens
        if style is not None:
            with NVTXContext("HunyuanDiT.forward.style_embedding"):
                style_embedding = self.style_embedder(style)
                extra_vec = torch.cat([extra_vec, style_embedding], dim=1)

        # Concatenate all extra vectors
        with NVTXContext("HunyuanDiT.forward.cat_all_extra_embedder"):
            c = t + self.extra_embedder(extra_vec)  # [B, D]

        # ========================= Forward pass through HunYuanDiT blocks =========================
        skips = []
        with NVTXContext("HunyuanDiT.forward.blocks"):
            for layer, block in enumerate(self.blocks):
                if controls is not None:
                    x = x + controls.pop(0)
                    # control = controls.pop(0)
                    # x = torch.concat([x, control], dim=-1)
                    # x = self.norm_list[layer](x)
                    # x = self.proj_list[layer](x)
                if layer > self.depth // 2:
                    skip = skips.pop()
        ...
        # ========================= Final layer =========================
        with NVTXContext("HunyuanDiT.forward.final_layer"):
            x = self.final_layer(x, c)                              # (N, L, patch_size ** 2 * out_channels)
        with NVTXContext("HunyuanDiT.forward.unpatchify"):
