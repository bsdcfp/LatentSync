...
class HunYuanReferenceNet(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(self,
        ...
        self.x_embedder = PatchEmbed(input_size, patch_size, in_channels + conditioning_channels, hidden_size)
    ...
    def from_dit(self, dit):
        ...
        x_embedder = torch.zeros((1408,9,2,2)) ##self.x_embedder.proj.weight.shape
        x_embedder[:,:4,...] = dit.x_embedder.proj.weight
        x_embedder[:,4:8,...] = dit.x_embedder.proj.weight
    ...
    def forward(self,
        ...
        # """
        # print("[INFO] HunyunaRefernece shape", x.shape, t.shape, condition.shape, encoder_hidden_states.shape, text_embedding_mask.shape, encoder_hidden_states_t5.shape, text_embedding_mask_t5.shape)
        ...
        # print('text_states',text_states.shape)
...
class HunYuanDiT(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def forward(self,
        ...
        return_dict: bool
            Whether to return a dictionary.
        """
        # print("[INFO] HunYuanDit shape", x.shape, t.shape, encoder_hidden_states.shape, text_embedding_mask.shape, encoder_hidden_states_t5.shape, text_embedding_mask_t5.shape)
        # print("[INFO] image_meta_size shape", image_meta_size.shape if image_meta_size else None)
        # print("[INFO] style shape", style.shape if style else None)
        # print("[INFO] cos_cis_img shape", cos_cis_img.shape if cos_cis_img is not None else None)
        # print("[INFO] sin_cis_img shape", sin_cis_img.shape if sin_cis_img is not None else None)
        # print("[INFO] return_dict", return_dict)
        # print("[INFO] controls shape", len(controls) if controls else None)
