...
class End2End(object):
    def __init__(self, args, models_root_path):
        ...
        # Initialize TeaCache flag - can be set via args or config
        self.enable_teacache = getattr(self.args, 'enable_teacache', False)
        ...
        if self.infer_mode in ['fa', 'torch']:
            ...
            #                                     log_fn=logger.info,
            #                                     ).half().to(self.device)
            self.controlnet = HunYuanReferenceNet(args,
                                       input_size=latent_size,
        ...
        if self.enable_teacache:
            from .teacache_hunyuandit import teacache_hunyuandit_forward
            # Replace the forward method
            self.model.forward = teacache_hunyuandit_forward.__get__(self.model, HunYuanDiT)
            
            # Set TeaCache attributes on the model instance (not class)
            self.model.enable_teacache = True
            self.model.cnt = 0
            self.model.num_steps = self.args.infer_steps
            self.model.rel_l1_thresh = 0.6  # 0.25 for 1.5x speedup, 0.4 for 1.8x speedup, 0.6 for 2.0x speedup, 0.8 for 2.25x speedup
            self.model.accumulated_rel_l1_distance = 0
            self.model.previous_modulated_input = None
            self.model.previous_residual = None
            logger.info(f"TeaCache enabled with threshold: {self.model.rel_l1_thresh}, steps: {self.model.num_steps}")
        # ========================================================================
    ...
    def load_torch_weights(self):
        ...
        #     controlnet_path = controlnet_dir / f"pytorch_model_{self.args.control_type}_{load_key}.pt"
        controlnet_path = self.args.controlnet_weight
        ...
        # self.controlnet = HunyuanDiT2DControlNetModel.from_pretrained("chuckma/hunyuan-v1.2-inpaint", torch_dtype=torch.float16)
    ...
    def calc_rope(self, height, width):
        ...
        # Move the rope tensors to the correct device
        if isinstance(rope, tuple):
            return rope[0].to(self.device), rope[1].to(self.device)
        else:
            return rope.to(self.device)
    ...
    def predict(self,
        ...
        if src_size_cond is None:
            ...
            image_meta_size = torch.as_tensor([size_cond] * batch_size, device=self.device)
        ...
        with NVTXContext("End2End.pipeline"):
