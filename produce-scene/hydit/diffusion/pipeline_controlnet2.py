...
import sys
# from productscene.trt_backend import TrtExecutor
import time
...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    # def load_trt(self, engine_root_path, batch_size=1, use_cuda_graph=False, verbose=False):
    #     print("[INFO] Start loading trt engine for StableDiffusionControlNetPipeline")
    #     self.backend = 'trt'
    #     self.verbose = verbose
    #     self.trt_executor = TrtExecutor(stages=['unet', 'control'], use_cuda_graph=use_cuda_graph, verbose=verbose)
    ...
    def encode_prompt(
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.encode_prompt.text_encoder"):
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.encode_prompt.prompt_embeds"):
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.encode_prompt.negative_prompt_embeds"):
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.encode_prompt.do_classifier_free_guidance"):
    ...
    def __call__(
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.encode_prompt"):
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.encode_prompt_t5"):
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.cat_prompt_embeds"):
            if do_classifier_free_guidance:
        ...
        if verbose:
            ...
            print("[INFO] Encode input prompt", "execution time {:.2f}".format(start.elapsed_time(end)), "ms")

        # 4. Prepare timesteps
        with NVTXContext("StableDiffusionControlNetPipeline.set_timesteps"):
            self.scheduler.set_timesteps(num_inference_steps, device=device)
            timesteps = self.scheduler.timesteps

        # 6. Prepare latent variables
        num_channels_latents = self.unet.config.in_channels
        with NVTXContext("StableDiffusionControlNetPipeline.prepare_latents"):
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.prepare_extra_step_kwargs"):
            extra_step_kwargs = self.prepare_extra_step_kwargs(generator, eta)

        with NVTXContext("StableDiffusionControlNetPipeline.vae.encode"):
                ...
                with NVTXContext("StableDiffusionControlNetPipeline.onestep"):
                    ...
                    if self.infer_mode in ["fa", "torch"]:
                        ...
                        if self.verbose:
                            ...
                            start.record()
                        with NVTXContext("StableDiffusionControlNetPipeline.onestep.controlnet"):

                            if self.backend == 'trt' and 'control' in self.trt_executor.stages:
                                param = {
                                ...
                                if self.trt_executor.nvtx_profile:
                                    nvtx_unet_encoder = nvtx.start_range(message='control', color='yellow')
                                    print("Start control", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                                out_dict = self.trt_executor.runEngine('control', param)
                                controls = [out_dict[i] for i in out_dict.keys() if 'control_' in i]
                                real_backend = 'trt'
                            else:
                            ...
                            if self.verbose:
                                ...
                                torch.cuda.synchronize()
                                if self.trt_executor.nvtx_profile:
                                    nvtx.end_range(nvtx_unet_encoder)
                                    print("End control", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                                print("[INFO] control", real_backend, "execution time {:.2f}".format(start.elapsed_time(end)), "ms")
                            ############################### BACKEND #################################
                            if verbose:
                                end.record()
                                torch.cuda.synchronize()
                                print("[INFO] controlnet {:.2f}".format(start.elapsed_time(end)), "ms")
                        ...
                        if self.verbose:
                            ...
                            start.record()
                        with NVTXContext("StableDiffusionControlNetPipeline.onestep.unet"):
                            if self.backend == 'trt' and 'unet' in self.trt_executor.stages:
                                # print("[INFO] Run trt engine")
                                param = {
                                    "x": latent_model_input,
                                    "t": t_expand,
                                    "encoder_hidden_states": prompt_embeds,
                                    ...
                                    "cos_cis_img": freqs_cis_img[0],
                                    "sin_cis_img": freqs_cis_img[1],
                                    **{"control_%d" % i: controls[i] for i in range(len(controls))},
                                }
                                if self.trt_executor.nvtx_profile:
                                    nvtx_unet = nvtx.start_range(message='unet', color='red')
                                    print("Start unet", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                                noise_pred =  self.trt_executor.runEngine('unet', param)['out_sample']
                                # print(noise_pred)
                                real_backend = 'trt'
                            ...
                            if self.verbose:
                                ...
                                torch.cuda.synchronize()
                                if self.trt_executor.nvtx_profile:
                                    print("End unet", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
                                    nvtx.end_range(nvtx_unet)
                                print("[INFO] unet", real_backend, "execution time {:.2f}".format(start.elapsed_time(end)), "ms")
                            ############################### BACKEND #################################
                            if verbose:
                                end.record()
                                torch.cuda.synchronize()
                                print("[INFO] unet {:.2f}".format(start.elapsed_time(end)), "ms")
                    ...
                    # perform guidance
                    with NVTXContext("StableDiffusionControlNetPipeline.onestep.guidance"):
                        if do_classifier_free_guidance:
                    ...
                    with NVTXContext("StableDiffusionControlNetPipeline.onestep.scheduler"):
        ...
        if verbose:
            ...
            start.record()
        with NVTXContext("StableDiffusionControlNetPipeline.decode"):
        ...
        with NVTXContext("StableDiffusionControlNetPipeline.postprocess"):
        ...
        if verbose:
            ...
            torch.cuda.synchronize()
            print("[INFO] decode {:.2f}".format(start.elapsed_time(end)), "ms")
        if not return_dict:
            return (image, has_nsfw_concept)
