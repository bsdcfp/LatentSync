...
def init_dist(launcher="pytorch", backend='nccl', port=28888, **kwargs):
...
def train(local_rank, num_processes, is_main_process, global_rank, args):
    if args.training_parts == "lora":
        args.use_ema = False
    ...
    model = HUNYUAN_DIT_MODELS[args.model](args,input_size=latent_size,log_fn=logger.info) 
    ...
    # optimizer = optimizer_cls([p for p in model.module.parameters() if p.requires_grad],lr=0,betas=(0.9, 0.999),eps=1e-8)
    import torch.optim as optim
    optimizer = optim.Adam(model.module.parameters(), lr=0.0)

    # lr_scheduler = get_scheduler(
    #     'constant',
    #     optimizer=optimizer,
    #     num_warmup_steps=args.warmup_num_steps * num_processes,
    #     num_training_steps=args.ckpt_every * num_processes,
    # )
...
def main(args):
    local_rank      = init_dist(port=28888)
    global_rank     = dist.get_rank()
    num_processes   = dist.get_world_size()
    # num_processes   = 0
    is_main_process = global_rank == 0
    train(local_rank, num_processes, is_main_process, global_rank, args)
