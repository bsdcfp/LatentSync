...
class MaskedStableDiffusionXLBrushNetPipeline(StableDiffusionXLBrushNetPipeline):
    ...
    def prepare_ip_adapter_image_embeds(
        self, ip_adapter_image, ip_adapter_image_embeds, fg_image, fg_mask, device, num_images_per_prompt, do_classifier_free_guidance,
        ...
        if ip_adapter_image_embeds is None:
            ...
            image_embeds = []
            image_embeds_mask = []
            for i, (single_ip_adapter_image, fg_single_ip_adapter_image,image_proj_layer) in enumerate(zip(
                ip_adapter_image, fg_image, self.unet.encoder_hid_proj.image_projection_layers
            )):
                output_hidden_state = not isinstance(image_proj_layer, ImageProjection)
                single_ip_adapter_mask = None if ip_adapter_image_mask is None else ip_adapter_image_mask[i]
                fg_single_ip_adapter_mask = None if fg_mask is None else fg_mask[i]
                ...
                fg_single_image_embeds, fg_single_negative_image_embeds, fg_embeds_mask, fg_negative_embeds_mask = self.encode_image(
                    fg_single_ip_adapter_image, device, 1, output_hidden_state, attention_mask=fg_single_ip_adapter_mask
                )
                ...
                fg_single_image_embeds = torch.stack([fg_single_image_embeds] * num_images_per_prompt, dim=0)
                fg_single_embeds_mask = torch.stack([fg_embeds_mask] * num_images_per_prompt, dim=0)
                fg_single_negative_image_embeds = torch.stack([fg_single_negative_image_embeds] * num_images_per_prompt, dim=0)
                fg_single_negative_embeds_mask = torch.stack([fg_negative_embeds_mask] * num_images_per_prompt, dim=0)

                single_image_embeds = torch.cat([single_image_embeds,fg_single_image_embeds], dim=2)
                single_negative_image_embeds = torch.cat([single_negative_image_embeds,fg_single_negative_image_embeds], dim=2)
