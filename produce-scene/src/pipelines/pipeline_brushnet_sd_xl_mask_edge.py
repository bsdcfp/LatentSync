...
class MaskedStableDiffusionXLBrushNetEdgePipeline(StableDiffusionXLBrushNetPipeline):
    ...
    def __call__(
        ...
        image_canny: PipelineImageInput = None,
        ...
        **kwargs,
        ...
        if isinstance(brushnet, BrushNetModel):
            ...
            original_canny = self.prepare_image(
                image=image_canny,
                width=width,
                height=height,
                batch_size=batch_size * num_images_per_prompt,
            ...
            original_canny=(original_canny.sum(1)[:,None,:,:] < 0).to(image.dtype)
        ...
        canny = torch.nn.functional.interpolate(
                    original_canny, 
                    size=(
                        conditioning_latents.shape[-2], 
                        conditioning_latents.shape[-1]
        ...
        conditioning_latents = torch.concat([conditioning_latents,canny,mask],1)
