...
def max_rect(mask):
    """ return the bounding box of the max contour in the mask
    args: 
        mask: np.ndarray, shape = [h, w, 3] | [h, w]
    returns:
        box: list, format = [x, y, w, h]
    """
    if len(mask.shape) != 2:
...
def main_rect(mask):
    """ return the bounding box that can surround all contours in the mask
    args: 
        mask: np.ndarray, shape = [h, w, 3] | [h, w]
    returns:
        box: list, format = [x, y, w, h]
