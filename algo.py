...
from oneservice import load_config
import yaml
from tensorrt.trt_latentsync.backends.snn_torch_backend import SNNTorchBackend
...
class ModelConfig:
    ...
    def __init__(self):
        ...
        self.config_file = os.path.join(self.auxiliary_dir, "latentsync_merge_all-v2/config.yaml")
        self.model_path = os.path.join(self.auxiliary_dir, "latentsync_merge_all-v2")
...
class ModelInferenceLocalV2(object):
    def __init__(self, ):
        self.config = ModelConfig()
        self.config_model = yaml.safe_load(open(self.config.config_file).read())["predictors"]
        self.config_model = load_config(
                        self.config_model["snn_torch_predictor"], "snn_torch_predictor"
                    )
        self.config_model.model_path = self.config.model_path
        self._engine = SNNTorchBackend(self.config_model)
...
class ModelInferenceLocal(object):
    ...
    def __call__(self,
        ...
        for i in range(num_inferences):
            ...
            chunk_faces_chunk = chunk_faces_chunk.to(torch.device('cuda:0')).float()
...
class AlgoModel(object):
    ...
    def __init__(self):
        ...
        self.video_feature_actor = VideoFeature()
        #self.model_inference_by_actor = ModelInferenceByRpc.bind()
        #self.model_inference_by_actor = ModelInferenceLocal.bind()
        #self.model_inference_by_actor = ModelInferenceLocal()
        self.model_inference_by_actor = ModelInferenceLocalV2()
        #self.postprocess_video_actor = PostprocessVideo.bind()
        self.postprocess_video_actor = PostprocessVideo()
        #self.merge_video_and_audio_actor = MergeVideoAndAudio.bind()
        self.merge_video_and_audio_actor = MergeVideoAndAudio()
    ...
    async def __call__(self, ctx):
        ...
        video_data = self.video_feature_actor.__call__(audio_features['audio_duration'])
        ...
        synced_video_frames = self.model_inference_by_actor.__call__(
        ...
        final_video_frames = self.postprocess_video_actor.__call__(
        ...
        video_out_path = self.merge_video_and_audio_actor.__call__(
