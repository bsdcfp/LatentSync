...
class SNNTorchBackend(BaseBackend):
    ...
    def _init_models(self, config: Dict):
        ...
        # 加载UNet配置 - 处理相对路径
        unet_config_path = config.get("unet_config_path", "latentsync/configs/unet/second_stage.yaml")
        if not os.path.isabs(unet_config_path):
            unet_config_path = os.path.join(current_dir, unet_config_path)
        unet_config = OmegaConf.load(unet_config_path)
        
        # 初始化调度器 - 处理相对路径
        scheduler_config_path = config.get("scheduler_config_path", "latentsync/configs")
        if not os.path.isabs(scheduler_config_path):
            scheduler_config_path = os.path.join(current_dir, scheduler_config_path)
