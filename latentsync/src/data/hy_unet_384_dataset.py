        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self.load_audio_data:
                    ...
                    mel = torch.zeros((1, 80, self.mel_window_length))  # 空占位符
                
                # 准备mask和masked_images
                gt, masked_gt, mask = image_processor.prepare_masks_and_masked_images(
                    continuous_frames, affine_transform=False
                )
...
class CustomUNetDataset_384_hy_0227_v3(CustomUNetDataset_384_hy_0227_v2):
    """
    UNet数据集v3版本 - 使用crop_face裁剪逻辑和新的mask
    """
    def __init__(self, train_data_dir: list, config):
        # 调用父类的初始化
        super().__init__(train_data_dir, config)
        
        # 使用新的mask
        self.mask_path = "/home/work/houyi/pj_25_q1/LatentSync/hy_test/crop_face/narrowed_mask/output_mask_09.png"
        print(f"使用新的mask路径: {self.mask_path}")
        
        # 直接使用load_fixed_mask函数加载新的mask
    ...
    def read_image(self, image_path):
        ...
        try:
            ...
            # 计算裁剪尺寸 - 使用crop_face的逻辑
            crop_size = int(min(width, height) * self.crop_scale)
            
            # 计算裁剪区域：底部对齐，水平居中
            x1 = width // 2 - crop_size // 2  # 水平居中
            y1 = height - crop_size  # 底部对齐
