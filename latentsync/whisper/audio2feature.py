...
class Audio2Feature:
    ...
    def _audio2feat_from_data(self, audio_data: bytes) -> torch.Tensor:
        """
        从音频二进制数据提取特征
        ...
        # 使用whisper模型提取特征
        result, origin_audio_frame = self.model.transcribe(audio_tensor)
    ...
    def _audio2feat(self, audio_path: str):
        ...
        result, origin_audio_frame = self.model.transcribe(audio_path)
        # print(f"origin_audio_frame: {origin_audio_frame}") # origin_audio_frame: 455
        embed_list = []
        for emb in result["segments"]:
            encoder_embeddings = emb["encoder_embeddings"]
            encoder_embeddings = encoder_embeddings.transpose(0, 2, 1, 3)
            ...
            embed_list.append(encoder_embeddings[:emb_end_idx])
        concatenated_array = torch.from_numpy(np.concatenate(embed_list, axis=0))
        return concatenated_array # 1250, 5, 384

    def audio2feat(self, audio_path_or_data, preloaded_audio=None):
        """
        从音频路径或二进制数据提取特征
        
        Args:
            audio_path_or_data: 音频文件路径（str）或音频二进制数据（bytes）
            preloaded_audio: 预加载的音频数据 (tuple of (audio_array, sr))，如果提供则跳过加载步骤
        ...
        if preloaded_audio is not None:
            # 使用预加载的音频数据
            audio_array, sr = preloaded_audio
            # 转换为whisper需要的格式
            audio_tensor = torch.from_numpy(audio_array)
            ...
            concatenated_array = torch.from_numpy(np.concatenate(embed_list, axis=0))
            return concatenated_array
        elif isinstance(audio_path_or_data, bytes):
            # 直接处理二进制数据
            return self._audio2feat_from_data(audio_path_or_data)
        elif isinstance(audio_path_or_data, str):
            # 处理文件路径
            if self.audio_embeds_cache_dir == "" or self.audio_embeds_cache_dir is None:
                return self._audio2feat(audio_path_or_data)

            audio_embeds_cache_path = os.path.join(self.audio_embeds_cache_dir, os.path.basename(audio_path_or_data) + ".pt")
            ...
            if os.path.isfile(audio_embeds_cache_path):
                try:
                    ...
                    audio_feat = self._audio2feat(audio_path_or_data)
            ...
            raise ValueError(f"不支持的音频输入类型: {type(audio_path_or_data)}, 期望 str 或 bytes")
