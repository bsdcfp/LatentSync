...
class LipsyncPipeline_online_0701(DiffusionPipeline):
    ...
    def _load_video_and_mask_data_from_index_list(self, video_index_list: List[dict], device, video_cache_dir = "templates/0612_online_templete_female_32fps_features", mask_base_dir: str = None, debug=False):
        """
        根据视频索引列表加载视频帧和特征数据
    ...
    def preprocess_audio_features(
        ...
        **kwargs,
        ...
        """
        音频特征提取和音频数据处理：处理所有音频相关的逻辑
        
        Args:
            audio_path_or_data: 音频路径（str）或音频二进制数据（bytes）
            video_fps: 视频帧率
            debug: 调试模式
            
        Returns:
            dict: 包含音频特征数据、音频样本和时长
        """
        whisper_chunks = None
        ...
        if isinstance(audio_path_or_data, bytes):
            # 处理二进制音频数据
            audio_array, sr = librosa.load(io.BytesIO(audio_path_or_data), sr=16000)
            audio_samples = torch.from_numpy(audio_array).float()
            audio_duration = len(audio_array) / sr
            preloaded_audio = (audio_array, sr)  # 保存预加载的音频数据
            
            # if debug:
            print(f"🎵 音频时长: {audio_duration} 秒 (从二进制数据)")
        else:
            # 处理文件路径
            audio_samples, audio_duration = read_audio(audio_path_or_data, return_duration=True)
            
            if debug:
                print(f"🎵 音频时长: {audio_duration} 秒 (从文件路径)")
        
        # 2. 音频特征提取（可能需要GPU）- 使用预加载的音频数据避免重复加载
        if debug:
            if isinstance(audio_path_or_data, bytes):
                print(f"🎵 开始音频特征提取（使用预加载数据）...")
            else:
                print(f"🎵 开始音频特征提取（从文件路径）...")
                
        whisper_feature = self.audio_encoder.audio2feat(audio_path_or_data, preloaded_audio=preloaded_audio)
    ...
    def preprocess_video_data(
        ...
        **kwargs,
        ...
        Args:
            video_cache_dir: 视频缓存目录
            audio_duration: 音频时长（秒）
            mask_base_dir: mask目录
            debug: 调试模式
        ...
        # print(f"self.device: {self.device}")
        if debug:
            print(f"📹 视频数据处理，基于音频时长: {audio_duration} 秒")
        ...
        # print(f"faces device: {faces[0].device}")
        # print(f"self.device: {self.device}")
    ...
    def preprocess_data_0701(
        ...
        **kwargs,
        ...
        # 子任务1: 音频特征提取和音频数据处理（可能需要GPU）
        if debug:
            print("🎵 步骤1: 音频特征提取和数据处理...")
        ...
        video_data = self.preprocess_video_data(
            video_cache_dir=video_cache_dir,
            audio_duration=audio_features['audio_duration'],
        ...
        if debug:
            ...
            print(f"   - 音频时长: {audio_features['audio_duration']:.2f}秒")
    ...
    def generate_video_online_0701(
        ...
        **kwargs,
        ...
        for i in range(preprocess_data['num_inferences']):
        ...
        time_end = time.time()
        print(f"🎉 视频生成完成，耗时: {time_end - time_begin:.2f}s, 音频数据大小: {len(audio_data)} bytes")
        return preprocess_data['original_faces_len']
