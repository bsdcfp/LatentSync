    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                continue
        #         # os.path.join(self.frames_dir, "align_frame")
        # 返回样本字典
        sample = dict(
            gt=gt,
            masked_gt=masked_gt,
            ...
            mel=mel,
            mask=mask,
            # video_path=self.sequences[sequence_id],  # 用序列名称替代视频路径
            # video_path=os.path.join(self.frames_dir, self.sequences[sequence_id]),  # 用序列名称替代视频路径
...
class CustomUNetDataset_384_hy_0227_v2(Dataset):
    def __init__(self, train_data_dir, config):
        """
        初始化自定义数据集，支持多个数据目录，采用帧图像+WAV的目录结构
        
        Args:
            train_data_dir: 训练数据根目录或目录列表
            config: 配置对象
        """
        print("===== 数据集初始化开始 =====")
        
        # 处理train_data_dir，确保是列表
        if isinstance(train_data_dir, str):
            self.data_dirs = [train_data_dir]
        else:
            self.data_dirs = list(train_data_dir)  # 确保转换为列表
        
        print(f"数据目录列表: {self.data_dirs}")
        
        # 加载所有序列
        self.all_sequences = []  # 所有序列的列表
        self.dir_indices = []    # 每个序列对应的数据目录索引
        self.sequence_paths = {} # 存储序列对应的完整路径信息
        
        for dir_idx, data_dir in enumerate(self.data_dirs):
            # 查找align_frame目录下的所有序列文件夹
            frames_dir = os.path.join(data_dir, "align_frame")
            audios_dir = os.path.join(data_dir, "audios")
            
            print(f"扫描数据目录 {dir_idx}: {data_dir}")
            ...
            if not os.path.exists(audios_dir):
                print(f"警告: 音频目录不存在 {audios_dir}")
                continue
            
            valid_sequences = []
            
            # 遍历align_frame下的所有子目录
            for sequence_name in os.listdir(frames_dir):
                sequence_dir = os.path.join(frames_dir, sequence_name)
                ...
                # 检查是否有足够的帧图像
                frame_files = [f for f in os.listdir(sequence_dir) if f.endswith(('.jpg', '.png', '.jpeg'))]
                frame_files.sort()  # 确保帧按顺序排列
                
                # 检查对应的音频文件
                audio_path = os.path.join(audios_dir, f"{sequence_name}.wav")
                
                if len(frame_files) >= 16 and os.path.exists(audio_path):
                    valid_sequences.append((sequence_name, sequence_dir, audio_path))
            
            print(f"从目录 {data_dir} 找到 {len(valid_sequences)} 个有效序列")
            
            # 添加序列和对应的目录索引
            for seq_name, frames_path, audio_path in valid_sequences:
                self.all_sequences.append(seq_name)
                self.dir_indices.append(dir_idx)
                self.sequence_paths[seq_name] = {
                    "frames_dir": frames_path,
                    "audio_path": audio_path,
                    "data_dir": data_dir
                }
        
        print(f"总共找到 {len(self.all_sequences)} 个序列")
        if len(self.all_sequences) == 0:
            raise ValueError("所有数据目录中都未找到有效的序列")
    ...
    def read_audio_file(self, audio_path):
        ...
        try:
            ...
            ar = AudioReader(audio_path, ctx=cpu(0), sample_rate=self.audio_sample_rate)
            audio_data = ar[:].asnumpy().squeeze(0)
            original_mel = melspectrogram(audio_data)
            return torch.from_numpy(original_mel)
        except Exception as e:
            print(f"读取音频文件失败: {e}")
            return None
    ...
    def get_sequence(self, sequence_id):
        """获取指定序列的连续帧和参考帧"""
        sequence_name = self.all_sequences[sequence_id]
        
        # 获取帧目录和音频文件路径
        frames_dir = self.sequence_paths[sequence_name]["frames_dir"]
        audio_path = self.sequence_paths[sequence_name]["audio_path"]
        ...
        try:
            ...
            if total_frames < self.num_frames * 2:
                print(f"序列帧数不足: {sequence_name}, 只有 {total_frames} 帧")
                return None, None, None, audio_path
            
            # 随机选择一个起始帧
            start_idx = random.randint(0, total_frames - self.num_frames - 1)
            
            # 读取连续帧
            continuous_frames = []
            for i in range(start_idx, start_idx + self.num_frames):
                img_path = os.path.join(frames_dir, frame_files[i])
            ...
            continuous_frames = np.array(continuous_frames)
            
            # 获取参考帧（从序列其他部分选择）
            ref_start = random.randint(0, total_frames - self.num_frames - 1)
            while abs(ref_start - start_idx) < self.num_frames:  # 确保参考帧与连续帧不重叠
                ref_start = random.randint(0, total_frames - self.num_frames - 1)
            
            ref_frames = []
            for i in range(ref_start, ref_start + self.num_frames):
                img_path = os.path.join(frames_dir, frame_files[i])
                img = cv2.imread(img_path)
                if img is None:
                    print(f"无法读取图像: {img_path}")
            ...
            return continuous_frames, ref_frames, start_idx, audio_path
            
        except Exception as e:
            print(f"加载序列失败: {sequence_name}, 错误: {e}")
            return None, None, None, audio_path
    ...
    def __getitem__(self, idx):
        """获取数据样本"""
        image_processor = getattr(self, f"image_processor_{self.worker_id}", None)
        if image_processor is None:
            image_processor = ImageProcessor(self.resolution, self.mask, mask_image=self.mask_image)
            setattr(self, f"image_processor_{self.worker_id}", image_processor)
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self.load_audio_data:
                    ...
                    mel_cache_path = os.path.join(self.audio_mel_cache_dir, f"{audio_basename}_mel.pt")
        ...
        sample = dict(
            ...
            mel=mel,
            mask=mask,
            video_path=self.sequence_paths[sequence_name]["audio_path"],
            start_idx=start_idx,
        )
